최근 1년간 회사에서 두번의 DB 마이그레이션을 진행했습니다.

MSSQL-> PostgreSQL -> Oracle로 전환하는 과정을 거쳤습니다.
오라클 전환할 때는 포스트그레 전환할 때 처럼 "쿼리만 조금 손보면 되겠지" 싶었는데 그렇지 않았네요. 
스키마 정의부터 락 전략, 성능 튜닝까지 손봐야 할 부분이 생각보다 많습니다!
